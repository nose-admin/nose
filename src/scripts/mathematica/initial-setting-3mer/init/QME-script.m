...
exxc1=1;
exxc2=1;
MMM[t_]=Evaluate[Simplify[((To4x4Formalism[\[Rho][t],-I/\[HBar] (Hm.\[Rho][t]-\[Rho][t].Transpose[Conjugate[Hm]])+1/\[HBar]^2 Sum[Ue[j].\[Rho][t].Transpose[Conjugate[\[CapitalLambda][j]]]+\[CapitalLambda][j].\[Rho][t].Ue[j],{j,1,Length[Hs]}],exxc1,exxc2]/.Equations).To4x4Formalism[S,Use[t].S.Use[-t],exxc1,exxc2]),t>0]];
(*Print["*"]
...
vyraz[t_]=MMM[t][[r]][[s]];
listOfM[r][s]=Table[If[0<timelist[[i]]<20\[Tau]c[1],Chop[vyraz[timelist[[i]]],10^-12],0] E^(-gg timelist[[i]]),{i,1,NN}];
];];
];
time2=AbsoluteTime[];
...
(*Print["@@@Eq",s," ",Re[Drop[res[s],-NN/2]],Im[Drop[res[s],-NN/2]]];*)
If[SECULAR,
Print["@@@EQ",s," ",Re[Drop[res[s],-NN/2]],Im[Drop[res[s],-NN/2]]],
Print["@@@Eq",s," ",Re[Drop[res[s],-NN/2]],Im[Drop[res[s],-NN/2]]],
Print["@@@Eq",s," ",Re[Drop[res[s],-NN/2]],Im[Drop[res[s],-NN/2]]]
...
ListPlot[ Re[res[4]],PlotRange->All,Joined->True]
ListPlot[ Im[res[4]],PlotRange->All,Joined->True]
ListPlot[ Re[res[5]],PlotRange->All,Joined->True]
ListPlot[ Im[res[5]],PlotRange->All,Joined->True]
ListPlot[ Re[res[6]],PlotRange->All,Joined->True]
ListPlot[ Im[res[6]],PlotRange->All,Joined->True]
ListPlot[ Re[res[7]],PlotRange->All,Joined->True]
ListPlot[ Im[res[7]],PlotRange->All,Joined->True]*)
