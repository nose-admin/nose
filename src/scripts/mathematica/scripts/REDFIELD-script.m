...
Cor[m_,n_,t_]:=Sum[(NovaCor[Part[tmpCor[m,n],i]][t]),{i,1,Length[tmpCor[m,n]]}];
...
Print["Equations:" ,Length[Equations]];
time2=AbsoluteTime[];
...
Print["Lambdas: ",Length[Lambdas]];
Initial=Table[(Lambdas[[i]]/.{t->0})==0,{i,1,Length[Lambdas]}];
Print["Other lists created"];


time1=AbsoluteTime[];
\[Lambda]ambdas=NDSolve[Join[Equations,Initial],Lambdas,{t,0,TIME}(*,WorkingPrecision->7*),MaxSteps->10^6][[1]];
